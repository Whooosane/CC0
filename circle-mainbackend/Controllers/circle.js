...
module.exports.createCircle = async (req, res) => {
  ...
  try {
    ...
    const invitePromises = phoneNumbers.map(async (phoneNumber) => {
      ...
      console.log(
        `Invite created for ${phoneNumber} with invite ID: ${invites._id}`
      );
      ...
      try {
        ...
        console.error(
          `Failed to send invitation to ${phoneNumber}:`,
          sendError
        );
      }
    });
    ...
    const eventTypesPromises = eventTypes.map(async (eventType) => {
      let existingEventType = await eventTypeModel.findOne({
        name: eventType.name,
    ...
    await userModel.findByIdAndUpdate(
      ownerId,
      { $addToSet: { ownedGroups: circle._id } },
      { session }
    );
...
module.exports.getCircleMembers = async (req, res) => {
  ...
  try {
    const circle = await circleModel
      .findById(circleId)
      .populate("members", "name phoneNumber _id profilePicture");
    if (!circle) {
...
module.exports.addCircleMember = async (req, res) => {
  ...
  try {
    ...
    if (circle.members.includes(memberId)) {
      return res
        .status(400)
        .json({ error: "Member is already part of the circle" });
    }
...
module.exports.addCircleMembers = async (req, res) => {
  ...
  try {
    ...
    if (circle.owner.toString() !== req.user._id.toString()) {
      return res
        .status(403)
        .json({ error: "Only the circle owner can add members" });
    }

    const newMembers = memberIds.filter(
      (memberId) => !circle.members.includes(memberId)
    );
...
module.exports.removeCircleMember = async (req, res) => {
  ...
  try {
    ...
    if (circle.owner.toString() !== req.user._id.toString()) {
      return res
        .status(403)
        .json({ error: "Only the circle owner can remove members" });
    }

    if (memberId === circle.owner.toString()) {
      return res
        .status(400)
        .json({ error: "Owner cannot be removed from the circle" });
    }
...
module.exports.getAllCircles = async (req, res) => {
  try {
    ...
    const circleWithLastMessage = await Promise.all(
...
module.exports.getCircleById = async (req, res) => {
  ...
  try {
    ...
    if (!circle.members.some((member) => member._id.equals(req.user._id))) {
      return res
        .status(403)
        .json({ error: "You are not a member of this circle" });
    }
...
module.exports.updateCircle = async (req, res) => {
  ...
  try {
    ...
    if (circle.owner.toString() !== req.user._id.toString()) {
      return res
        .status(403)
        .json({ error: "Only the circle owner can update the circle" });
    }
